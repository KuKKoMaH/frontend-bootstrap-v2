...
/***/ function(module, exports, __webpack_require__) {
	...
	var $block = $('.' + _styles2.default.form.block);

	var fieldNames = [{ name: 'name', type: 'string', required: true }, { name: 'email', type: 'email' }, { name: 'phone', type: 'phone', required: true }, { name: 'message', type: 'string' }];

	var fields = fieldNames.map(function (field) {
	  var $input = $block.find('[name="' + field.name + '"]');
	  var $wrapper = $input.parent();
	  return {
	    wrapper: $wrapper,
	    input: $input,
	    error: $wrapper.find('.' + _styles2.default.form.message),
	    type: field.type,
	...
	new _Validator2.default({
	  ...
	  classes: {
	    error: _styles2.default.form.error
	  },
	  fields: fields
	});
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = ({"styles":{"index":{},"layout":{},"header":{"header":"src-modules-header-header"},"slider":{"s1":"src-modules-slider-s1","s2":"src-modules-slider-s2"},"form":{"block":"src-modules-form-block","message":"src-modules-form-message","error":"src-modules-form-error","input":"src-modules-form-input"}}}).styles;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var re_email = /^[a-z0-9!#$%&'*+\/=?^_`{|}~.-]+@[a-z0-9]([a-z0-9-]*[a-z0-9])?(\.[a-z0-9]([a-z0-9-]*[a-z0-9])?)*$/i;
	var re_phone = /\+7 \(\d{3}\) \d{3}-\d{2}-\d{2}/i;
	var re_date = /^\d{2}\.\d{2}\.\d{4}$/i;
	var allowed_filetypes = ['pdf', 'txt', 'doc', 'docx', 'xls', 'xlsx', 'jpg', 'jpeg', 'png', 'gif'];
	...
	var Validator = function () {
	  ...
	  _createClass(Validator, [{
	    ...
	    value: function onSubmitForm(e) {
	      ...
	      var fieldStates = this.fields.map(function (field) {
	        return _this.validateField(field, true);
	      });
	      if (fieldStates.some(function (state) {
	        return !state;
	      })) return;
	    ...
	    key: 'bindEvents',
	    value: function bindEvents(fields) {
	      var _this2 = this;
	      ...
	      fields.forEach(function (field) {
	        ...
	        field.input.on('blur', function () {
	          _this2.validateField(field, true);
	        });
	      });
	    }
	    ...
	    key: 'testField',
	    value: function testField(field, value) {
	...
	Validator.rules = {
	  email: function email(val, field) {
	    return re_email.test(val);
	  },
	  phone: function phone(val, field) {
	    return re_phone.test(val);
	  },
	  date: function date(val, field) {
	    var d = val.split('.');
	    return re_date.test(val) && !isNaN(new Date(d[2] + '-' + d[1] + '-' + d[0]).getTime());
	  },
	  string: function string(val, field) {
	    return val.length > 0;
	  },
	  checkbox: function checkbox(val, field) {
	    return field.input.prop('checked');
	  },
	  file: function file(val, field) {
